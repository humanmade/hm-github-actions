...
jobs:
  plugin-security-review:
    ...
    steps:
      ...
      - name: Plugin Security Review
        uses: humanmade/hm-github-actions/.github/actions/plugin-security-review@a6e9df8092386f601122bf8f0415659b2836105a
        with:
...
- `local_command`: Optional display-only template for the "Run locally" hint in the review. Provide a project convenience wrapper such as `composer check-security {path}`, where `{path}` is replaced with each flagged plugin directory. This changes only the suggested command text shown to the developer, not the scan the action actually runs, so this alias must run the same standard passed as `security_standard` or a developer could get unexpectedly different local results versus the findings reported in CI. Defaults to the raw `vendor/bin/phpcs --standard=<security_standard> <path>`.
